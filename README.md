...
> [!IMPORTANT]
> スクリプト上では`template`というユーザ名のアカウントを既定としてみなすように動作します。別のユーザ名を指定する場合は、スクリプト8行目の該当箇所を変更します。
...
- スクリプトを実行せずに新規ユーザを作成してください
 - 新規ユーザがカスタマイズされていないものであれば、`C:\Users\Default.bk`フォルダを`Default`に変更してください。
 - カスタマイズされたユーザが新規作成された場合は、Windowsのデフォルトユーザデータが消失しています。PCの初期化を検討してください。

### Windows Defaultユーザのプロファイルが見つかりません。フォルダの状態を確認して再実行してください。
`C:\Users\Default`フォルダが見つかりません。
ユーザプロファイルの基礎となるデータがなくなっています。
- `C:\Users\Default.bk`フォルダがある場合、そのフォルダ名を`Default`に変更してください。
- `C:\Users\Default`および`C:\Users\Default.bk`フォルダどちらも存在しない場合は、PCを初期化してください。
